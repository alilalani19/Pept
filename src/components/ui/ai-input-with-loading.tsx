...
interface AIInputWithLoadingProps {
  ...
  autoAnimate?: boolean;
  isLoading?: boolean;
  statusText?: { loading: string; ready: string };
}
...
export function AIInputWithLoading({
  ...
  statusText = { loading: "AI is thinking...", ready: "Ready to submit!" },
  ...
  const handleSubmit = async () => {
    ...
    // Only use internal timeout if not externally controlled
    ...
    <div className={cn("w-full py-4", className)}>
      <div className="relative max-w-xl w-full mx-auto flex items-start flex-col gap-2">
        <div className="relative max-w-xl w-full mx-auto">
          ...
          <button
            ...
            className={cn(
              ...
              loading ? "bg-none" : "bg-black/5 dark:bg-white/5"
            )}
            type="button"
            disabled={loading}
          >
        ...
        <p className="pl-4 h-4 text-xs mx-auto text-black/70 dark:text-white/70">
          {loading ? statusText.loading : statusText.ready}
        </p>
      </div>
    </div>
  );
